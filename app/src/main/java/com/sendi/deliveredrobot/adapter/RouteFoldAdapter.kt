...
import com.sendi.deliveredrobot.holder.FoldMainVH
...
class RouteFoldAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    protected fun mainMenuLayoutId(): Int{
        return R.layout.item_route_main_menu
    }
    ...
    protected fun subMenuLayoutId(): Int{
        return R.layout.item_binding_route
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (itemViewType) {
            ITEM_MAIN_TYPE -> {
                val mainMenuVH: FoldMainVH = holder as FoldMainVH
            ...
            ITEM_SUB_TYPE -> {
                val subMenuVH: RouteFoldSubVH = holder as RouteFoldSubVH
...
//        }
     ...
     */
    protected open fun bindSubMenuData(subMenuVH: RouteFoldSubVH, subMenuManger: FoldSelMapModel) {
        subMenuVH.ivSelected.apply {
            if(subMenuManger.mSelect){
                visibility = View.VISIBLE
            }else{
                visibility = View.GONE
            }
        }
        if(mMainMenus[subMenuManger.parentIndex].mSubMenu.size - 1 == subMenuManger.index){
            subMenuVH.viewRouteUnderline.visibility = View.GONE
        }else{
            subMenuVH.viewRouteUnderline.visibility = View.VISIBLE
        }
    ...
    protected open fun bindMainMenuData(mainMenuVH: FoldMainVH, mFoldSelMapModel: FoldSelMapModel) {
    ...
    fun setData(dataList: ArrayList<AllMapRelationshipModel>) {
        ...
        if (dataList != null) {
            for ((index,item) in dataList.withIndex()){
                ...
                for ((subIndex,mSubItem) in item.routeMap!!.withIndex()) {
                    if(item.mSubMap.id == mSubItem.subMapId){
                        var subData : FoldSelMapModel = FoldSelMapModel(mSubItem.id, mSubItem.name?:"",index,item.selectRouteId == mSubItem.id,true,false,ArrayList<FoldSelMapModel>(),realSubIndex)
                ...
                var mFoldSelMapModel : FoldSelMapModel = FoldSelMapModel(item.mSubMap.id, item.mSubMap.name!!, -1, item.selected, true,
                    false, mSubDataList,index,item.selectRouteId,item.selectRouteName)
